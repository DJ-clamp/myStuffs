...
$.message = "";
let opts = {
  username: process.env.BBS7USERNAME || "",
  password: process.env.BBS7PASSWORD || "",
  baseUrl: process.env.BBS7BASEURL || "", //论坛首页地址 结尾带上”/”,
  qdxq: "kx", //签到时使用的心情
  todaysay: "开心~~~", //想说的话
};
...
class generateRequests {
  ...
  // 账号登录地址
  loginPageUrl() {
    return this.getBaseUrl() + "logging.php?action=login&loginsubmit=yes&floatlogin=yes";
  }
  // 账号信息提交地址
  loginSubmitUrl() {
    return (
      this.getBaseUrl() +
      "logging.php?action=login&loginsubmit=yes&floatlogin=yes&inajax=13"
    );
  }
  //签到页面地址
  signPageUrl() {
  ...
  //红包页面地址
  moneySubmitUrl() {
    return this.getBaseUrl() + "plugin.php?identifier=get_money&module=money&action=money_get";
  }
  //红包页面地址
  moneyPageUrl() {
    return this.getBaseUrl() + "plugin.php?identifier=get_money&module=money&action=money_get&hid=36";
  }
  ...
  monePageWithCookie(cookie) {
    return {
      url: this.moneyPageUrl(),
      headers: {
        cookie: cookie,
        "user-agent":
          "Mozilla/5.0 (Linux; Android 10; Redmi K30) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.83 Mobile Safari/537.36",
  ...
  moneySubmit(cookie, hashString, hid) {
    return {
      url: this.moneySubmitUrl(),
      body: `formhash=${hashString}&hid=${hid}&hsubject=&money_get=%C1%EC%C8%A1%B8%C3%BA%EC%B0%FC`,
      headers: {
        cookie: cookie,
        "user-agent":
          "Mozilla/5.0 (Linux; Android 10; Redmi K30) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.83 Mobile Safari/537.36",
  ...
  async GetMoney(cookie = null) {
    let ck = cookie ? cookie : this.getCookie();
    try {
      let { hashString, hid } = await this.getMoneyPage(ck);
      let { message } = await this.postMoneySubmit(ck, hashString, hid)
      return message || "";
    } catch (error) {
      return e;
    }
  ...
  async postMoneySubmit(cookie, hashString, hid) {
    return new Promise(async (resolve, reject) => {
      $.post(this.moneySubmit(cookie, hashString, hid), async (err, response, data) => {
        if (data.indexOf("您今天已经签到过") > -1) {
          reject(($.message += "今天已签过到\r\n"));
        } else {
          $.message += "当前零钱增加" + 15 + "元";
...
(async () => {
  ...
  await Discuz.Login();
  let message = await Discuz.GetMoney(Discuz.cookie);
  $.msg(message);
})();
